...
class Preview_NIFTI_Module(ScanModule):
    def __init__(self,module_name=DEFAULT_MODULE_NAME,directory=DEFAULT_TPM_PATH,email=DEFAULT_EMAIL,Text_report=DEFAULT_TEXT_REPORT,masimatlabpath=DEFAULT_MASIMATLAB_PATH,resourcename=DEFAULT_RESOURCE):
    ...
    def run(self,xnat,projectName,subject,experiment,scan):
        ...
        if Scan.resource('SNAPSHOTS').exists():
            ...
            if Scan.resource(self.resourcename).exists():
                if len(Scan.resource(self.resourcename).files().get()) > 0:
                    print "      -downloading "+self.resourcename+"..."
                    
                    dl,filename=XnatUtils.download_biggest_resources(Scan.resource(self.resourcename),self.directory)
                    
                    if not dl:
                        print '      -ERROR: '+self.resourcename+' file size is zero.'
                    else:
                        smGif = os.path.join(self.directory,os.path.splitext(os.path.basename(filename))[0] + "_sm.gif")
                        lgGif = os.path.join(self.directory,os.path.splitext(os.path.basename(filename))[0] + "_lg.gif")
                ...
                print '    *No resource '+self.resourcename
